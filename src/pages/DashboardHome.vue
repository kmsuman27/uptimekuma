<template>
        ...
        <div v-if="$route.name === 'DashboardHome'" class="monitor-hub">
            ...
            <div class="hub-action-bar">
                <div class="hub-actions">
                    <!-- Add Monitor -->
                    <router-link to="/add" class="hub-btn hub-btn-primary">
                        <font-awesome-icon icon="plus" />
                        <span>Add Monitor</span>
                    </router-link>

                    <!-- Bulk Import -->
                    <button class="hub-btn" type="button" @click="showBulkImportModal = true">
                        <font-awesome-icon icon="upload" />
                        Bulk Import
                    </button>

                    <!-- CSV Upload -->
                    <button type="button" class="hub-btn" @click="showCSVImportModal = true">
            ...
            <!-- Empty State -->
            <div v-if="monitorCount === 0" class="empty-state">
                ...
                <p>Add your first monitor to start monitoring uptime and availability</p>
    ...
    />
    <CSVImportModal :show="showCSVImportModal" @close="showCSVImportModal = false" @imported="handleCSVImportResult" />
</template>
    ...
    methods: {
        ...
        handleBulkImportResult(result) {
            ...
            if (result.successCount > 0) {
                this.$root.toastSuccess(`${result.successCount} monitor(s) imported successfully`);
            }

            if (result.failedCount > 0) {
                this.$root.toastError(`${result.failedCount} monitor(s) failed to import`);
            }
        },
